import React from 'react'
import './styles/ContactCTA.css'

const ContactCTA: React.FC = () => {

  return (
    <section id="contact" className="contact-minimal">
      <div className="contact-minimal-container">
        {/* Left side - Contact Information */}
        <div className="contact-info-side">
          {/* Logo & Tagline */}
          <div className="contact-logo-section">
            <img 
              src="/Logo_gouldingandco_white.png" 
              alt="Goulding & Co" 
              className="contact-logo"
            />
            <p className="contact-tagline">Strategic Advisory for Cambodia Market Entry</p>
          </div>

          {/* Contact Methods Icons */}
          <div className="contact-icons">
            <h2>Get in Touch</h2>
            <div className="icons-row">
              <a href="https://t.me/gouldingandco" target="_blank" rel="noopener noreferrer" className="contact-icon">
                <svg viewBox="0 0 24 24" fill="currentColor">
                  <path d="M9.78,18.65L10.06,14.42L17.74,7.5C18.08,7.19 17.67,7.04 17.22,7.31L7.74,13.3L3.64,12C2.76,11.75 2.75,11.14 3.84,10.7L19.81,4.54C20.54,4.21 21.24,4.72 20.96,5.84L18.24,18.65C18.05,19.56 17.5,19.78 16.74,19.36L12.6,16.3L10.61,18.23C10.38,18.46 10.19,18.65 9.78,18.65Z"/>
                </svg>
                <span>Telegram</span>
              </a>
              <a href="https://wa.me/855969829922" target="_blank" rel="noopener noreferrer" className="contact-icon">
                <svg viewBox="0 0 24 24" fill="currentColor">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/>
                </svg>
                <span>WhatsApp</span>
              </a>
              <a href="mailto:brett@gouldingandco.com" className="contact-icon">
                <svg viewBox="0 0 24 24" fill="currentColor">
                  <path d="M20,8L12,13L4,8V6L12,11L20,6M20,4H4C2.89,4 2,4.89 2,6V18A2,2 0 0,0 4,20H20A2,2 0 0,0 22,18V6C22,4.89 21.1,4 20,4Z"/>
                </svg>
                <span>Email</span>
              </a>
            </div>
          </div>

          {/* Office Location */}
          <div className="office-location">
            <h3>Phnom Penh Office</h3>
            <p className="office-address">
              Level 26, Unit 32<br/>
              Le Conde Building<br/>
              Street 352, BKK1<br/>
              Phnom Penh, Cambodia
            </p>
          </div>
          
          {/* Footer Copyright */}
          <div className="contact-footer">
            <p className="copyright">© 2024 Goulding & Co Strategic Advisory</p>
            <p className="footer-tagline">Your Gateway to Cambodia's Opportunities</p>
          </div>
        </div>

        {/* Right side - Full screen Map */}
        <div className="map-container-fullscreen">
          <iframe 
            src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d1954.4918722528883!2d104.92084561531436!3d11.552703091790954!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2sGoulding+%26+Co+Strategic+Advisory!5e0!3m2!1sen!2skh!4v1635842957849!5m2!1sen!2skh&ll=11.552703,104.920846&z=17&q=Goulding+%26+Co,+Le+Conde+Building,+Level+26,+Unit+32,+Street+352,+BKK1,+Phnom+Penh"
            width="100%"
            height="100%"
            style={{ border: 0 }}
            allowFullScreen
            loading="lazy"
            referrerPolicy="no-referrer-when-downgrade"
          />
        </div>
      </div>
    </section>
  )
}

export default ContactCTA